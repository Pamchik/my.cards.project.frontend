...
import { useGetPersoVendorQuery, useUpdatePersoVendorMutation } from "../../../../store/api/persoVendorApiSlice";
import { useAllComponentParamsReset } from "../../../hooks/useComponentDataReset";
import useTimeoutManager from "../../../hooks/useTimeoutManager";


interface IPersoVendorMainInfo {
    selectedID: number
}

const PersoVendorMainInfo = ({
    selectedID
}: IPersoVendorMainInfo) => {

    const componentName = 'PersoVendorMainInfo'
    ...
    const {data: persoVendorData, isFetching: isFetchingPersoVendorData, error: errorPersoVendorData, refetch: refetchPersoVendorData} = useGetPersoVendorQuery({id: selectedID})
    const [updatePersoVendor, {}] = useUpdatePersoVendorMutation()
    ...
    useEffect(() => {
        if (componentsAPIUpdate.includes('persoVendorData')) {
            refetchPersoVendorData()
            funcChangeCancelSet(true)
            dispatch(deleteComponentsAPIUpdate(['persoVendorData']))
        }
    }, [componentsAPIUpdate]);
    ...
    useEffect(() => {
        ...
        if (isComponentPrepared) {
            if (!isFetchingPersoVendorData) {
                if (!errorPersoVendorData && persoVendorData) {
                    const myData = funcConvertToFieldDataType(persoVendorData[0])
    ...
    }, [selectedID, isComponentPrepared, persoVendorData, isFetchingPersoVendorData]);
    ...
    async function funcSaveFields () {
        ...
        if (currentId) {
            ...
            if (isValid.isAllFieldsValid) {
                ...
                await updatePersoVendor({...myData, id: currentId}).unwrap()    
                .then((res) => {
...
export default PersoVendorMainInfo
...
import { ILaminationsData, useGetLaminationsQuery } from "../../../../store/api/laminationApiSlice";
...
const LaminationsList = ({
    ...
    const componentName = "LaminationsList"
    ...
    const {data: laminationsData, isFetching: isFetchingLaminationsData, error: errorLaminationsData, refetch: refetchLaminationsData} = useGetLaminationsQuery(undefined)
    ...
    useEffect(() => {
        if (componentsAPIUpdate.includes('laminationsData')) {
            refetchLaminationsData()
            dispatch(deleteComponentsAPIUpdate(['laminationsData']))
    ...
    useEffect(() => {
        if (!isComponentPrepared) {
            componentPreparing({ loadingFieldsData: ['name', 'vendor', 'status', 'arrLaminations'], fieldsParamsData: initFieldParams });
    ...
    useEffect(() => {
        if (isComponentPrepared) {
            if (!isFetchingLaminationsData) {
                setManagedTimeout(() => {
                    dispatch(setLoadingStatus({componentName, data: {status: false, isSuccessful: true, fields: ['arrLaminations']}}))
                }, 1000)
            } else {
                dispatch(setLoadingStatus({componentName, data: {status: true, fields: ['arrLaminations']}}))
            }
        }
    }, [isFetchingLaminationsData, isComponentPrepared]);

    const [sortedLaminationsData, setSortedLaminationsData] = useState<ILaminationsData[]>([])
    
    useEffect(() => {
        if (laminationsData && isComponentPrepared) {
            ...
            const newData = funcSortedDataByValue(funcFilteredDataByValues(laminationsData, fieldsCheckValues), 'name_eng')
            setSortedLaminationsData(newData)
            if (!newData.find(item => item.id === selectedID)) {
                setSelectedID(null)
            }
        } else {
            setSortedLaminationsData([])
            setSelectedID(null)
        }
    },[componentData?.objInputAndChangedData, laminationsData, isComponentPrepared])

    let itemsList;
    if (loadingProcess?.['arrLaminations'].status) {
        itemsList = (
        ...
        if (errorLaminationsData) {
            ...
            itemsList = (
                <>{sortedLaminationsData.map((item) => (
                    <ListBlock
                        key={item.id}
                        title={item.name_rus}
                        onClick={() => {
        ...
        {isComponentPrepared && (
            <MainInfoBlock myStyleMain={{maxWidth: '300px', borderRight: '1px solid #bebebe'}}>
                <BtnBlock>
                    <ButtonMain
                        onClick={() => {
                            dispatch(setModalProps({componentName: 'CreateNewLaminationModal', data: {
...
export default LaminationsList
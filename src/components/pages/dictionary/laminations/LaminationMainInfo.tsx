...
import { useGetLaminationQuery, useUpdateLaminationMutation } from "../../../../store/api/laminationApiSlice";
import { useAllComponentParamsReset } from "../../../hooks/useComponentDataReset";
import useTimeoutManager from "../../../hooks/useTimeoutManager";

interface ILaminationMainInfo {
    selectedID: number
}

const LaminationMainInfo = ({
    selectedID
}: ILaminationMainInfo) => {

    const componentName = 'LaminationMainInfo'
    ...
    const {data: laminationData, isFetching: isFetchingLaminationData, error: errorLaminationData, refetch: refetchLaminationData} = useGetLaminationQuery({id: selectedID})
    ...
    const [updateLamination, {}] = useUpdateLaminationMutation()
    ...
    useEffect(() => {
        if (componentsAPIUpdate.includes('laminationData')) {
            refetchLaminationData()
            funcChangeCancelSet(true)
            dispatch(deleteComponentsAPIUpdate(['laminationData']))
    ...
    useEffect(() => {
        ...
        if (isComponentPrepared) {
            if (!isFetchingLaminationData) {
                if (!errorLaminationData && laminationData) {
                    const myData = funcConvertToFieldDataType(laminationData[0])
    ...
    }, [selectedID, isComponentPrepared, laminationData, isFetchingLaminationData]);

    useEffect(() => {
        dispatch(setLoadingStatus({componentName, data: {status: true, fields: ['vendor']}}))
        if (isComponentPrepared && !isFetchingVendorsData && !isFetchingLaminationData && !errorLaminationData) {
            if (!errorVendorsData) {
                setManagedTimeout(() => {
    ...
    }, [selectedID, isComponentPrepared, isFetchingLaminationData, isFetchingVendorsData]);
    ...
    async function funcSaveFields () {
        ...
        if (currentId) {
            ...
            if (isValid.isAllFieldsValid) {
                ...
                await updateLamination({...myData, id: currentId}).unwrap()    
                .then((res) => {
...
export default LaminationMainInfo
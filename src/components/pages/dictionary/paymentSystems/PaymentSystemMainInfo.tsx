...
import { useGetPaymentSystemQuery, useUpdatePaymentSystemMutation } from "../../../../store/api/paymentSystemApiSlice";
import { useAllComponentParamsReset } from "../../../hooks/useComponentDataReset";
import useTimeoutManager from "../../../hooks/useTimeoutManager";


interface IPaymentSystemMainInfo {
    selectedID: number
}

const PaymentSystemMainInfo = ({
    selectedID
}: IPaymentSystemMainInfo) => {

    const componentName = 'PaymentSystemMainInfo'
    ...
    const {data: paymentSystemData, isFetching: isFetchingPaymentSystemData, error: errorPaymentSystemData, refetch: refetchPaymentSystemData} = useGetPaymentSystemQuery({id: selectedID})
    const [updatePaymentSystem, {}] = useUpdatePaymentSystemMutation()
    ...
    useEffect(() => {
        if (componentsAPIUpdate.includes('paymentSystemData')) {
            refetchPaymentSystemData()
            funcChangeCancelSet(true)
            dispatch(deleteComponentsAPIUpdate(['paymentSystemData']))
        }
    }, [componentsAPIUpdate]);
    ...
    useEffect(() => {
        ...
        if (isComponentPrepared) {
            if (!isFetchingPaymentSystemData) {
                if (!errorPaymentSystemData && paymentSystemData) {
                    const myData = funcConvertToFieldDataType(paymentSystemData[0])
    ...
    }, [selectedID, isComponentPrepared, paymentSystemData, isFetchingPaymentSystemData]);
    ...
    async function funcSaveFields () {
        ...
        if (currentId) {
            ...
            if (isValid.isAllFieldsValid) {
                ...
                await updatePaymentSystem({...myData, id: currentId}).unwrap()    
                .then((res) => {
...
export default PaymentSystemMainInfo
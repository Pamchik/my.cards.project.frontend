...
import { useGetProductCategoryQuery, useUpdateProductCategoryMutation } from "../../../../store/api/productCategoryApiSlice";
import { useAllComponentParamsReset } from "../../../hooks/useComponentDataReset";
import useTimeoutManager from "../../../hooks/useTimeoutManager";


interface IProductCategoryMainInfo {
    selectedID: number,
}

const ProductCategoryMainInfo = ({
    selectedID,
}: IProductCategoryMainInfo) => {

    const componentName = 'ProductCategoryMainInfo'
    ...
    const {data: productCategoryData, isFetching: isFetchingProductCategoryData, error: errorProductCategoryData, refetch: refetchProductCategoryData} = useGetProductCategoryQuery({id: selectedID})
    ...
    useEffect(() => {
        if (componentsAPIUpdate.includes('productCategoryData')) {
            refetchProductCategoryData()
            funcChangeCancelSet(true)
            dispatch(deleteComponentsAPIUpdate(['productCategoryData']))
        }
    }, [componentsAPIUpdate]);
    ...
    useEffect(() => {
        ...
        if (isComponentPrepared) {
            if (!isFetchingProductCategoryData) {
                if (!errorProductCategoryData && productCategoryData) {
                    const myData = funcConvertToFieldDataType(productCategoryData[0])
    ...
    }, [selectedID, isComponentPrepared, productCategoryData, isFetchingProductCategoryData]);
    ...
    async function funcSaveFields () {
        ...
        if (currentId) {
            ...
            if (isValid.isAllFieldsValid) {
                ...
                await updateProductCategory({...myData, id: currentId}).unwrap()    
                .then((res) => {
...
export default ProductCategoryMainInfo
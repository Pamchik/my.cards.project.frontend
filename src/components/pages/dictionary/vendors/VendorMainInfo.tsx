...
import { useGetVendorQuery, useUpdateVendorMutation } from "../../../../store/api/vendorApiSlice";
import { useAllComponentParamsReset } from "../../../hooks/useComponentDataReset";
import useTimeoutManager from "../../../hooks/useTimeoutManager";

interface IVendorMainInfo {
    selectedID: number
}

const VendorMainInfo = ({
    selectedID
}: IVendorMainInfo) => {

    const componentName = 'VendorMainInfo'
    ...
    const {data: vendorData, isFetching: isFetchingVendorData, error: errorVendorData, refetch: refetchVendorData} = useGetVendorQuery({id: selectedID})
    ...
    useEffect(() => {
        if (componentsAPIUpdate.includes('vendorData')) {
            refetchVendorData()
            funcChangeCancelSet(true)
            dispatch(deleteComponentsAPIUpdate(['vendorData']))
        }
    }, [componentsAPIUpdate]);
    ...
    useEffect(() => {
        ...
        if (isComponentPrepared) {
            if (!isFetchingVendorData) {
                if (!errorVendorData && vendorData) {
                    const myData = funcConvertToFieldDataType(vendorData[0])
    ...
    }, [selectedID, isComponentPrepared, vendorData, isFetchingVendorData]);
    ...
    async function funcSaveFields () {
        ...
        if (currentId) {
            ...
            if (isValid.isAllFieldsValid) {
                ...
                await updateVendor({...myData, id: currentId}).unwrap()    
                .then((res) => {

...
import { useGetVendorEmployeeQuery, useUpdateVendorEmployeeMutation } from "../../../../store/api/vendorEmployeeApiSlice";
import { useAllComponentParamsReset } from "../../../hooks/useComponentDataReset";
import useTimeoutManager from "../../../hooks/useTimeoutManager";


interface IVendorEmployeeMainInfo {
    selectedID: number,
}

const VendorEmployeeMainInfo = ({
    selectedID,
}: IVendorEmployeeMainInfo) => {

    const componentName = 'VendorEmployeeMainInfo'
    ...
    const {data: vendorEmployeeData, isFetching: isFetchingVendorEmployeeData, error: errorVendorEmployeeData, refetch: refetchVendorEmployeeData} = useGetVendorEmployeeQuery({id: selectedID})
    ...
    useEffect(() => {
        if (componentsAPIUpdate.includes('vendorEmployeeData')) {
            refetchVendorEmployeeData()
            funcChangeCancelSet(true)
            dispatch(deleteComponentsAPIUpdate(['vendorEmployeeData']))
        }
    }, [componentsAPIUpdate]);
    ...
    useEffect(() => {
        ...
        if (isComponentPrepared) {
            if (!isFetchingVendorEmployeeData) {
                if (!errorVendorEmployeeData && vendorEmployeeData) {
                    const myData = funcConvertToFieldDataType(vendorEmployeeData[0])
    ...
    }, [selectedID, isComponentPrepared, vendorEmployeeData, isFetchingVendorEmployeeData]);
...
export default VendorEmployeeMainInfo
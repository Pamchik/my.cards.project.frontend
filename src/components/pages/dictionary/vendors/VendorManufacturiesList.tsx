...
import { IVendorManufacturiesData, useGetVendorManufacturiesQuery } from "../../../../store/api/vendorManufacturiesApiSlice";
...
interface IVendorManufacturiesList {
    setSelectedID: (id: number | null) => void,
    selectedID: number | null
    globalElementID: number
}

const VendorManufacturiesList = ({
    setSelectedID,
    selectedID,
    globalElementID
}: IVendorManufacturiesList) => {

    const componentName = "VendorManufacturiesList"
    ...
    const {data: vendorManufacturiesData, isFetching: isFetchingVendorManufacturiesData, error: errorVendorManufacturiesData, refetch: refetchVendorManufacturiesData} = useGetVendorManufacturiesQuery({vendor: globalElementID})
    ...
    useEffect(() => {
        if (componentsAPIUpdate.includes('vendorManufacturiesData')) {
            refetchVendorManufacturiesData()
            dispatch(deleteComponentsAPIUpdate(['vendorManufacturiesData']))
        }
    }, [componentsAPIUpdate]);
    ...
    useEffect(() => {
        if (!isComponentPrepared) {
            componentPreparing({ loadingFieldsData: ['name', 'status', 'arrVendorManufacturies'], fieldsParamsData: initFieldParams });
    ...
    useEffect(() => {
        dispatch(setLoadingStatus({ componentName, data: { status: true, fields: ['arrVendorManufacturies'] } }))
        dispatch(setLoadingStatus({componentName, data: {status: true, fields: Object.keys(initFieldParams)}}))
        if (isComponentPrepared) {
            setManagedTimeout(() => {
                dispatch(setLoadingStatus({componentName, data: {status: false, isSuccessful: true, fields: ['name', 'status']}}))
            }, 1000)
            if (!isFetchingVendorManufacturiesData) {
                setManagedTimeout(() => {
                    dispatch(setLoadingStatus({componentName, data: {status: false, isSuccessful: true, fields: ['arrVendorManufacturies']}}))
                }, 1000)
            } else {
                dispatch(setLoadingStatus({componentName, data: {status: true, fields: ['arrVendorManufacturies']}}))
            }
    ...
    }, [isFetchingVendorManufacturiesData, isComponentPrepared, globalElementID]);

    const [sortedVendorManufacturiesData, setSortedVendorManufacturiesData] = useState<IVendorManufacturiesData[]>([])

    useEffect(() => {
        if (vendorManufacturiesData && isComponentPrepared) {
            ...
            const newData = funcSortedDataByValue(funcFilteredDataByValues(vendorManufacturiesData, fieldsCheckValues), 'name')
            setSortedVendorManufacturiesData(newData)
            if (!newData.find(item => item.id === selectedID)) {
                setSelectedID(null)
            }
        } else {
            setSortedVendorManufacturiesData([])
            setSelectedID(null)
        }
    },[componentData?.objInputAndChangedData, vendorManufacturiesData, isComponentPrepared])
       
    let itemsList;
    if (loadingProcess?.['arrVendorManufacturies'].status) {
        itemsList = (
        ...
        if (errorVendorManufacturiesData) {
            ...
            itemsList = (
                <>{sortedVendorManufacturiesData.map((item) => (
                    <ListBlock
                        key={item.id}
                        title={item.name}
                        onClick={() => {
...
export default VendorManufacturiesList
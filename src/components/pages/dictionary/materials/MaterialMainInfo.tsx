...
import { useGetMaterialQuery, useUpdateMaterialMutation } from "../../../../store/api/materialApiSlice";
import { useGetProductTypesQuery } from "../../../../store/api/productTypeApiSlice";
import { useAllComponentParamsReset } from "../../../hooks/useComponentDataReset";
import useTimeoutManager from "../../../hooks/useTimeoutManager";

interface IMaterialMainInfo {
    selectedID: number
}
...
const MaterialMainInfo = ({
    ...
    const componentName = 'MaterialMainInfo'
    ...
    const {data: materialData, isFetching: isFetchingMaterialData, error: errorMaterialData, refetch: refetchMaterialData} = useGetMaterialQuery({id: selectedID})
    ...
    useEffect(() => {
        if (componentsAPIUpdate.includes('materialData')) {
            refetchMaterialData()
            funcChangeCancelSet(true)
            dispatch(deleteComponentsAPIUpdate(['materialData']))
    ...
    useEffect(() => {
        ...
        if (isComponentPrepared) {
            if (!isFetchingMaterialData) {
                if (!errorMaterialData && materialData) {
                    const myData = funcConvertToFieldDataType(materialData[0])
    ...
    }, [selectedID, isComponentPrepared, materialData, isFetchingMaterialData]);

    useEffect(() => {
        dispatch(setLoadingStatus({componentName, data: {status: true, fields: ['product_type']}}))
        if (isComponentPrepared && !isFetchingProductTypesData && !isFetchingMaterialData && !errorMaterialData) {
    ...
    }, [selectedID, isComponentPrepared, isFetchingMaterialData, isFetchingProductTypesData]);
    ...
    async function funcSaveFields () {
        ...
        if (currentId) {
            ...
            if (isValid.isAllFieldsValid) {
                ...
                await updateMaterial({...myData, id: currentId}).unwrap()    
                .then((res) => {

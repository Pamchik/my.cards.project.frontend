...
import { IProcessingsData, useGetProcessingsQuery } from "../../../../store/api/processingApiSlice";
...
interface IProcessingsList {
    setSelectedID: (id: number | null) => void,
    selectedID: number | null
}

const ProcessingsList = ({
    setSelectedID,
    selectedID
}: IProcessingsList) => {

    const componentName = "ProcessingsList"
    ...
    useEffect(() => {
        if (componentsAPIUpdate.includes('processingsData')) {
            refetchProcessingsData()
            dispatch(deleteComponentsAPIUpdate(['processingsData']))
        }
    }, [componentsAPIUpdate]);
    ...
    useEffect(() => {
        if (!isComponentPrepared) {
            componentPreparing({ loadingFieldsData: ['name', 'status', 'arrProcessings'], fieldsParamsData: initFieldParams });
    ...
    useEffect(() => {
        if (isComponentPrepared) {
            if (!isFetchingProcessingsData) {
                setManagedTimeout(() => {
                    dispatch(setLoadingStatus({componentName, data: {status: false, isSuccessful: true, fields: ['arrProcessings']}}))
                }, 1000)
            } else {
                dispatch(setLoadingStatus({componentName, data: {status: true, fields: ['arrProcessings']}}))
            }
        }
    }, [isFetchingProcessingsData, isComponentPrepared]);

    const [sortedProcessingsData, setSortedProcessingsData] = useState<IProcessingsData[]>([])
    
    useEffect(() => {
        if (processingsData && isComponentPrepared) {
            ...
            const newData = funcSortedDataByValue(funcFilteredDataByValues(processingsData, fieldsCheckValues), 'name')
            setSortedProcessingsData(newData)
            if (!newData.find(item => item.id === selectedID)) {
                setSelectedID(null)
            }
        } else {
            setSortedProcessingsData([])
            setSelectedID(null)
        }
    },[componentData?.objInputAndChangedData, processingsData, isComponentPrepared])

    let itemsList;
    if (loadingProcess?.['arrProcessings'].status) {
        itemsList = (
        ...
        if (errorProcessingsData) {
            ...
            itemsList = (
                <>{sortedProcessingsData.map((item) => (
                    <ListBlock
                        key={item.id}
                        title={item.name}
                        onClick={() => {

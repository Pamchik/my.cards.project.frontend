...
import { useGetProcessingQuery, useUpdateProcessingMutation } from "../../../../store/api/processingApiSlice";
import { useAllComponentParamsReset } from "../../../hooks/useComponentDataReset";
import useTimeoutManager from "../../../hooks/useTimeoutManager";


interface IProcessingMainInfo {
    selectedID: number
}

const ProcessingMainInfo = ({
    selectedID
}: IProcessingMainInfo) => {

    const componentName = 'ProcessingMainInfo'
    ...
    const {data: processingData, isFetching: isFetchingProcessingData, error: errorProcessingData, refetch: refetchProcessingData} = useGetProcessingQuery({id: selectedID})
    ...
    useEffect(() => {
        if (componentsAPIUpdate.includes('processingData')) {
            refetchProcessingData()
            funcChangeCancelSet(true)
            dispatch(deleteComponentsAPIUpdate(['processingData']))
        }
    }, [componentsAPIUpdate]);
    ...
    useEffect(() => {
        ...
        if (isComponentPrepared) {
            if (!isFetchingProcessingData) {
                if (!errorProcessingData && processingData) {
                    const myData = funcConvertToFieldDataType(processingData[0])
    ...
    }, [selectedID, isComponentPrepared, processingData, isFetchingProcessingData]);
    ...
    async function funcSaveFields () {
        ...
        if (currentId) {
            ...
            if (isValid.isAllFieldsValid) {
                ...
                await updateProcessing({...myData, id: currentId}).unwrap()    
                .then((res) => {
...
export default ProcessingMainInfo
...
import { useGetMifareQuery, useUpdateMifareMutation } from "../../../../store/api/mifareApiSlice";
import { useAllComponentParamsReset } from "../../../hooks/useComponentDataReset";
import useTimeoutManager from "../../../hooks/useTimeoutManager";


interface IMifareMainInfo {
    selectedID: number
}

const MifareMainInfo = ({
    selectedID
}: IMifareMainInfo) => {

    const componentName = 'MifareMainInfo'
    ...
    const {data: mifareData, isFetching: isFetchingMifareData, error: errorMifareData, refetch: refetchMifareData} = useGetMifareQuery({id: selectedID})
    const [updateMifare, {}] = useUpdateMifareMutation()
    ...
    useEffect(() => {
        if (componentsAPIUpdate.includes('mifareData')) {
            refetchMifareData()
            funcChangeCancelSet(true)
            dispatch(deleteComponentsAPIUpdate(['mifareData']))
        }
    }, [componentsAPIUpdate]);
    ...
    useEffect(() => {
        ...
        if (isComponentPrepared) {
            if (!isFetchingMifareData) {
                if (!errorMifareData && mifareData) {
                    const myData = funcConvertToFieldDataType(mifareData[0])
    ...
    }, [selectedID, isComponentPrepared, mifareData, isFetchingMifareData]);
    ...
    async function funcSaveFields () {
        ...
        if (currentId) {
            ...
            if (isValid.isAllFieldsValid) {
                ...
                await updateMifare({...myData, id: currentId}).unwrap()    
                .then((res) => {

...
import { useGetProductTypeQuery, useUpdateProductTypeMutation } from "../../../../store/api/productTypeApiSlice";
import { useAllComponentParamsReset } from "../../../hooks/useComponentDataReset";
import useTimeoutManager from "../../../hooks/useTimeoutManager";

interface IProductTypeMainInfo {
    selectedID: number
}

const ProductTypeMainInfo = ({
    selectedID
}: IProductTypeMainInfo) => {

    const componentName = 'ProductTypeMainInfo'
    ...
    const {data: productTypeData, isFetching: isFetchingProductTypeData, error: errorProductTypeData, refetch: refetchProductTypeData} = useGetProductTypeQuery({id: selectedID})
    ...
    useEffect(() => {
        if (componentsAPIUpdate.includes('productTypeData')) {
            refetchProductTypeData()
            funcChangeCancelSet(true)
            dispatch(deleteComponentsAPIUpdate(['productTypeData']))
    ...
    useEffect(() => {
        ...
        if (isComponentPrepared) {
            if (!isFetchingProductTypeData) {
                if (!errorProductTypeData && productTypeData) {
                    const myData = funcConvertToFieldDataType(productTypeData[0])
    ...
    }, [selectedID, isComponentPrepared, productTypeData, isFetchingProductTypeData]);

    useEffect(() => {
        dispatch(setLoadingStatus({componentName, data: {status: true, fields: ['vendor']}}))
        if (isComponentPrepared && !isFetchingVendorsData && !isFetchingProductTypeData && !errorProductTypeData) {
            if (!errorVendorsData) {
                setManagedTimeout(() => {
    ...
    }, [selectedID, isComponentPrepared, isFetchingProductTypeData, isFetchingVendorsData]);
    ...
    async function funcSaveFields () {
        ...
        if (currentId) {
            ...
            if (isValid.isAllFieldsValid) {
                ...
                await updateProductType({...myData, id: currentId}).unwrap()    
                .then((res) => {
...
export default ProductTypeMainInfo
...
interface IDeliveryFromVendorInfoData {
    selectedID: number
}

const DeliveryFromVendorInfo = ({
    selectedID,
}: IDeliveryFromVendorInfoData) => {

    const componentName = 'DeliveryFromVendorInfo'
    ...
    const {data: deliveriesInfoData, isFetching: isFetchingDeliveriesInfoData, error: errorDeliveriesInfoData, refetch: refetchDeliveriesInfoData} = useGetDeliveriesInfoQuery({line_number: selectedID, company_type: 'vendor', deleted: 'False'});
    ...
    const initFieldParams: IFieldParams = {
        ...
        'qty_received': {isRequired: false, type: 'text'},
    ...
    const [totalReceivedIntQty, setTotalReceivedIntQty] = useState<number>(0)
    useEffect(() => {
        if (deliveriesInfoData && deliveriesInfoData.length > 0) {
            ...
            setTotalReceivedIntQty(totalQty)
            setTotalReceivedQty(
    ...
    useEffect(() => {
        if (projectData && projectData.length > 0 && projectData[0].product_qty_to_vendor) {
            const percent = Math.round(+totalReceivedIntQty / +projectData[0].product_qty_to_vendor * 100)
            setDeviation((100 - percent) * -1)
        }
    }, [totalReceivedIntQty, projectData]);

    const [vendorQty, setVendorQty] = useState<string>('0')
    useEffect(() => {
        if (projectData && projectData[0]?.product_qty_to_vendor) {
            setVendorQty(funcNumberWithThousandSeparator(projectData[0].product_qty_to_vendor))
        } else {
            setVendorQty('0')
        }
    }, [projectData]);
            ...
            <ContentBlock myStyleMain={{flex: '0 0 auto'}} myStyleContent={{display: 'flex', flexDirection: 'row'}}>
                <ContentBlock title={'Данные по количеству:'} myStyleMain={{maxWidth: '50%'}} line={true}>
                    <BlockInput
                        onChange={() => {}}
                        fieldName={'qty'}
                        title={'Количество заказанных карт'}
                    ...
                    <BlockInput
                        ...
                        title={'Количество полученных карт'}
                        type={fieldParams?.['qty_received']?.type as IInputTypes}
                        value={totalReceivedQty}
                        placeholder={''}
                        skeletonLoading={loadingProcess?.['qty_received']}
                        isReadOnly={true}
                    />   
                    <BlockInput
                        onChange={() => {}}
...
export default DeliveryFromVendorInfo
...
import { useAddMaterialMutation, useUpdateMaterialMutation } from "../../store/api/materialApiSlice";
import { useGetProductTypesQuery } from "../../store/api/productTypeApiSlice";
import { useAllComponentParamsReset } from "../hooks/useComponentDataReset";
import useTimeoutManager from "../hooks/useTimeoutManager";


const CreateNewMaterialModal = () => {

    const componentName = 'CreateNewMaterialModal'
    ...
    async function funcSaveFields () {
        ...
        if (currentId) {
            ...
            if (isValid.isAllFieldsValid) {
                dispatch(deleteFieldInvalid({componentName}))
                const myData = componentData.objChangedData
                await addMaterial({...myData}).unwrap()    
                .then((res) => {
                    dispatch(setSavingStatus({componentName, data: {status: true, isSuccessful: true}}))
                    dispatch(deleteAllComponentData({componentName}))
                    setManagedTimeout(() => { 
        ...
        {isComponentPrepared && (<>
            ...
            >
                ...
                <MainModalTopBlock>
                    ...
                    <MainModalText modalTitle={`${componentData?.objInputData?.['id'] ? 'Изменение Материала' : 'Создание нового Материала'}`}/>
...
export default CreateNewMaterialModal
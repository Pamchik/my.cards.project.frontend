...
import { useAddPaymentSystemMutation, useUpdatePaymentSystemMutation } from "../../store/api/paymentSystemApiSlice";
import { useAllComponentParamsReset } from "../hooks/useComponentDataReset";
import useTimeoutManager from "../hooks/useTimeoutManager";


const CreateNewPaymentSystemModal = () => {

    const componentName = 'CreateNewPaymentSystemModal'
    ...
    const [addPaymentSystem, {}] = useAddPaymentSystemMutation()
    ...
    async function funcSaveFields () {
        ...
        if (currentId) {
            ...
            if (isValid.isAllFieldsValid) {
                dispatch(deleteFieldInvalid({componentName}))
                const myData = componentData.objChangedData
                await addPaymentSystem({...myData}).unwrap()    
                .then((res) => {
                    dispatch(setSavingStatus({componentName, data: {status: true, isSuccessful: true}}))
                    dispatch(deleteAllComponentData({componentName}))
                    setManagedTimeout(() => { 
        ...
        {isComponentPrepared && (<>
            ...
            >
                ...
                <MainModalTopBlock>
                    ...
                    <MainModalText modalTitle={`${componentData?.objInputData?.['id'] ? 'Изменение Платежной системы' : 'Создание Платежной системы'}`}/>
...
export default CreateNewPaymentSystemModal
...
import { useAddProductCategoryMutation, useUpdateProductCategoryMutation } from "../../store/api/productCategoryApiSlice";
import { useAllComponentParamsReset } from "../hooks/useComponentDataReset";
import useTimeoutManager from "../hooks/useTimeoutManager";


const CreateNewProductCategoryModal = () => {

    const componentName = 'CreateNewProductCategoryModal'
    ...
    async function funcSaveFields () {
        ...
        if (currentId) {
            ...
            if (isValid.isAllFieldsValid) {
                dispatch(deleteFieldInvalid({componentName}))
                const myData = componentData.objChangedData
                await addProductCategory({...myData}).unwrap()    
                .then((res) => {
                    dispatch(setSavingStatus({componentName, data: {status: true, isSuccessful: true}}))
                    dispatch(deleteAllComponentData({componentName}))
                    setManagedTimeout(() => { 
        ...
        {isComponentPrepared && (<>
            ...
            >
                ...
                <MainModalTopBlock>
                    ...
                    <MainModalText modalTitle={`${componentData?.objInputData?.['id'] ? 'Изменение Категории продукта' : 'Создание Категории продукта'}`}/>
...
export default CreateNewProductCategoryModal
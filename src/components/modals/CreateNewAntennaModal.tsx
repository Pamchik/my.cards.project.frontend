...
import { useAddAntennaMutation, useUpdateAntennaMutation } from "../../store/api/antennaApiSlice";
import { useGetVendorsQuery } from "../../store/api/vendorApiSlice";
import { useAllComponentParamsReset } from "../hooks/useComponentDataReset";
import useTimeoutManager from "../hooks/useTimeoutManager";


const CreateNewAntennaModal = () => {

    const componentName = 'CreateNewAntennaModal'
    ...
    const [addAntenna, {}] = useAddAntennaMutation()
    ...
    async function funcSaveFields () {
        ...
        if (currentId) {
            ...
            if (isValid.isAllFieldsValid) {
                dispatch(deleteFieldInvalid({componentName}))
                const myData = componentData.objChangedData
                await addAntenna({...myData}).unwrap()    
                .then((res) => {
                    dispatch(setSavingStatus({componentName, data: {status: true, isSuccessful: true}}))
                    dispatch(deleteAllComponentData({componentName}))
                    setManagedTimeout(() => { 
        ...
        {isComponentPrepared && (<>
            ...
            >
                ...
                <MainModalTopBlock>
                    ...
                    <MainModalText modalTitle={`${componentData?.objInputData?.['id'] ? 'Изменение Антенны' : 'Создание Антенны'}`}/>
...
export default CreateNewAntennaModal
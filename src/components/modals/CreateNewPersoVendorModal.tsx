...
import { useAddPersoVendorMutation, useUpdatePersoVendorMutation } from "../../store/api/persoVendorApiSlice";
...
const CreateNewPersoVendorModal = () => {

    const componentName = 'CreateNewPersoVendorModal'
    ...
    const [addPersoVendor, {}] = useAddPersoVendorMutation()
    ...
    async function funcSaveFields () {
        ...
        if (currentId) {
            ...
            if (isValid.isAllFieldsValid) {
                dispatch(deleteFieldInvalid({componentName}))
                const myData = componentData.objChangedData
                await addPersoVendor({...myData}).unwrap()    
                .then((res) => {
                    dispatch(setSavingStatus({componentName, data: {status: true, isSuccessful: true}}))
                    dispatch(deleteAllComponentData({componentName}))
                    setManagedTimeout(() => { 
        ...
        {isComponentPrepared && (<>
            ...
            >
                ...
                <MainModalTopBlock>
                    ...
                    <MainModalText modalTitle={`${componentData?.objInputData?.['id'] ? 'Изменение Персо вендора' : 'Создание Персо вендора'}`}/>
...
export default CreateNewPersoVendorModal
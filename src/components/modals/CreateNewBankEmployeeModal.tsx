...
import { useAddBankEmployeeMutation, useUpdateBankEmployeeMutation } from "../../store/api/bankEmployeeApiSlice";
...
const CreateNewBankEmployeeModal = () => {

    const componentName = 'CreateNewBankEmployeeModal'
    ...
    const [addBankEmployee, {}] = useAddBankEmployeeMutation()
    ...
    async function funcSaveFields () {
        ...
        if (currentId) {
            ...
            if (isValid.isAllFieldsValid) {
                dispatch(deleteFieldInvalid({componentName}))
                const myData = componentData.objChangedData
                await addBankEmployee({...myData}).unwrap()    
                .then((res) => {
                    dispatch(setSavingStatus({componentName, data: {status: true, isSuccessful: true}}))
                    dispatch(deleteAllComponentData({componentName}))
                    setManagedTimeout(() => { 
        ...
        {isComponentPrepared && (<>
            ...
            >
                ...
                <MainModalTopBlock>
                    ...
                    <MainModalText modalTitle={`${componentData?.objInputData?.['id'] ? 'Изменение сотрудника Банка' : 'Создание сотрудника Банка'}`}/>
...
export default CreateNewBankEmployeeModal